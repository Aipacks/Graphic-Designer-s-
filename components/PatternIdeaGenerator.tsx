...
import { generatePatternIdeas } from '../services/geminiService';
import { PatternIdea } from '../types';
...
interface PatternIdeaGeneratorProps {
  isAuthenticated: boolean;
  onAuthRequired: () => void;
}

export const PatternIdeaGenerator: React.FC<PatternIdeaGeneratorProps> = ({ isAuthenticated, onAuthRequired }) => {
  const [theme, setTheme] = useState<string>('');
  const [ideas, setIdeas] = useState<PatternIdea[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [copySuccess, setCopySuccess] = useState(false);

  const [savedItems, setSavedItems] = useState<SavedPatternIdeas[]>([]);
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem('savedPatternIdeas');
  ...
  useEffect(() => {
    if (!isInitialLoad) {
      localStorage.setItem('savedPatternIdeas', JSON.stringify(savedItems));
    }
  ...
  const handleGenerate = async () => {
    ...
    if (!theme) {
      setError('Please provide a theme for the patterns.');
      return;
    }
    setError(null);
    setLoading(true);
    ...
    try {
      const result = await generatePatternIdeas(theme);
      setIdeas(result);
    } catch (err: any) {
  ...
  const handleSave = () => {
    ...
    if (ideas.length > 0) {
      const newSave: SavedPatternIdeas = {
  ...
  const handleLoad = (item: SavedPatternIdeas) => {
    setTheme(item.theme);
    setIdeas(item.ideas);
    setCurrentItemId(item.id);
    setShowHistory(false);
    document.getElementById('pattern-tool')?.scrollIntoView({ behavior: 'smooth' });
  };
  ...
  const handleCopy = () => {
    const textToCopy = ideas.map(idea => `Name: ${idea.name}\nDescription: ${idea.description}`).join('\n\n');
  ...
  const handleDownload = () => {
    const textToDownload = ideas.map(idea => `# ${idea.name}\n\n${idea.description}`).join('\n\n---\n\n');
    const blob = new Blob([textToDownload], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `pattern_ideas_${theme.replace(/\s/g, '_')}.txt`;
    a.click();
  ...
  const features = [
      {
        ...
        title: "Unique Motifs & Themes",
        description: "Get creative concepts for pattern motifs, color palettes, and overall styles that you won't find anywhere else."
      },
      {
        ...
        description: "Each idea comes with a rich description, perfect for using as a creative brief or as a prompt for AI image generators."
      },
      {
        icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>,
        title: "Perfect for Any Medium",
        description: "Whether for textiles, wallpaper, digital backgrounds, or branding, get ideas that can be adapted for any surface."
      },
  ];

  const testimonials = [
      {
        quote: "As a surface pattern designer, this tool is my go-to for breaking creative blocks. I can enter a theme like 'Autumn in Japan' and get three amazing, fully-formed ideas instantly.",
        name: "Aiko Tanaka",
        title: "Textile Designer"
      },
      {
        quote: "I use this to brainstorm background patterns for websites and apps. It's so much faster than searching for inspiration, and the ideas are always unique and on-trend.",
        name: "Ben Carter",
   ...
   const faqs = [
      {
        ...
        a: "Be creative and specific! Try combining styles and subjects, like 'Art Deco Tropical Birds', 'Minimalist Scandinavian Forest', or 'Retro 80s Arcade Carpet'."
      },
      {
        q: "How many ideas do I get?",
        a: "The tool provides three distinct pattern ideas for every theme you enter, each with a detailed description of its style, motifs, and color palette."
      },
      {
        q: "Can I use these ideas to generate actual pattern images?",
        a: "Yes! The detailed descriptions are perfect for use as prompts in text-to-image AI tools to visualize the pattern concepts."
      },
  ];
  ...
  return (
    ...
    <div className="animate-fade-in -m-6 md:-m-8">
      ...
      <section className="text-center py-12 px-4 bg-background">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-4xl sm:text-5xl font-extrabold text-text mb-4 leading-tight">
            Design Seamless Patterns That <span className="text-primary">Delight</span>.
          </h1>
          <p className="max-w-2xl mx-auto text-lg text-muted mb-8">
            Never start from a blank canvas. Enter any theme and get three unique, detailed concepts for seamless patterns, complete with motif and color palette ideas.
          </p>
            <button onClick={() => document.getElementById('pattern-tool')?.scrollIntoView({ behavior: 'smooth' })} className="bg-primary text-white font-bold py-3 px-8 rounded-lg transition-transform hover:scale-105 shadow-lg">
              Generate an Idea
          </button>
        </div>
      </section>
      ...
      {/* The Tool Section */}
      <section id="pattern-tool" className="py-16 bg-surface px-4">
        <div className="container mx-auto max-w-4xl flex flex-col items-center">
          <div className="w-full flex justify-between items-center mb-4">
            <h2 className="text-3xl font-bold">Pattern Idea Generator</h2>
          ...
          <p className="text-center mb-6 text-muted max-w-2xl">Get inspired to create unique seamless patterns. Enter a theme to get detailed concepts.</p>
          ...
          <div className="w-full max-w-xl flex flex-col gap-4 mb-6">
            <input
              ...
              placeholder="e.g., 'Tropical Jungle', 'Mid-Century Modern Geometric'"
      ...
      <section className="py-16 bg-surface px-4">
        <div className="container mx-auto text-center">
            <h2 className="text-3xl font-bold mb-10">From a Spark of an Idea to a Finished Pattern</h2>
      ...
      <section className="py-16 text-center bg-surface px-4">
          <div className="container mx-auto">
              <h2 className="text-3xl font-bold mb-4">Ready to Create Your Next Bestselling Pattern?</h2>
              <p className="text-muted max-w-xl mx-auto mb-8">Stop searching and start creating. Generate your next great idea now.</p>
              <button onClick={() => document.getElementById('pattern-tool')?.scrollIntoView({ behavior: 'smooth' })} className="bg-primary text-white font-bold py-3 px-8 rounded-lg transition-transform hover:scale-105 shadow-lg">
    ...
    <div className={`fixed top-0 right-0 h-full w-full max-w-sm bg-surface shadow-2xl transform transition-transform duration-300 ${showHistory ? 'translate-x-0' : 'translate-x-full'} z-50 flex flex-col`}>
        ...
        <div className="flex-grow overflow-y-auto p-4">
            {savedItems.length > 0 ? (
                ...
                <p className="text-center text-muted mt-8">No saved pattern ideas yet.</p>
            )}
        </div>
    </div>
