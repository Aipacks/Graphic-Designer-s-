...
import { generateIllustrationIdeas } from '../services/geminiService';
import { IllustrationIdea } from '../types';
...
interface SavedIllustrationIdeas {
  id: string;
  timestamp: number;
  theme: string;
  ideas: IllustrationIdea[];
}
...
interface IllustrationIdeaGeneratorProps {
  isAuthenticated: boolean;
  onAuthRequired: () => void;
}

export const IllustrationIdeaGenerator: React.FC<IllustrationIdeaGeneratorProps> = ({ isAuthenticated, onAuthRequired }) => {
  ...
  const [savedItems, setSavedItems] = useState<SavedIllustrationIdeas[]>([]);
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem('savedIllustrationIdeas');
  ...
  useEffect(() => {
    if (!isInitialLoad) {
      localStorage.setItem('savedIllustrationIdeas', JSON.stringify(savedItems));
    }
  ...
  const handleGenerate = async () => {
    ...
    if (!theme) {
      setError('Please provide a theme for the illustrations.');
      return;
    }
    setError(null);
    setLoading(true);
    ...
    try {
      const result = await generateIllustrationIdeas(theme);
      setIdeas(result);
    } catch (err: any) {
  ...
  const handleSave = () => {
    if (ideas.length > 0) {
      const newSave: SavedIllustrationIdeas = {
  ...
  const handleLoad = (item: SavedIllustrationIdeas) => {
    setTheme(item.theme);
    setIdeas(item.ideas);
    setCurrentItemId(item.id);
    setShowHistory(false);
    document.getElementById('illustration-tool')?.scrollIntoView({ behavior: 'smooth' });
  };
  ...
  const handleCopy = () => {
    const textToCopy = ideas.map(idea => `Concept: ${idea.concept}\nDescription: ${idea.description}`).join('\n\n');
  ...
  const handleDownload = () => {
    const textToDownload = ideas.map(idea => `Concept: ${idea.concept}\nDescription: ${idea.description}`).join('\n\n');
    const blob = new Blob([textToDownload], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `illustration_ideas_${theme.replace(/\s/g, '_')}.txt`;
    a.click();
  ...
  const features = [
      {
        ...
        title: "Detailed Art Direction",
        description: "Get more than just a title. Each idea comes with a rich description covering style, color palette, and composition."
      },
      {
        icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" /></svg>,
        title: "Explore Any Theme",
        description: "From 'Cyberpunk City at Night' to 'Enchanted Forest Library', our AI can generate compelling concepts for any theme you can imagine."
      },
      {
        ...
        description: "The perfect tool to break through creative blocks. Generate a set of ideas and find the perfect spark for your next masterpiece."
      },
  ];

  const testimonials = [
      {
        quote: "This is my secret weapon for brainstorming editorial illustrations. I can plug in the theme of an article and get three brilliant, fully-formed concepts in seconds.",
        name: "Jasmine Lee",
        title: "Freelance Illustrator"
      },
      {
        quote: "As a concept artist, speed is everything. This tool helps me generate a wide range of ideas quickly, so I can present more options and nail the creative direction faster.",
        name: "David Chen",
   ...
   const faqs = [
      {
        q: "How does the AI come up with these ideas?",
        a: "The AI has been trained on a vast range of art and design concepts. It analyzes your theme and generates new, unique combinations of styles, subjects, and compositions based on its training."
      },
      {
        q: "Can I use these ideas to generate an actual image?",
        a: "Yes! The detailed descriptions are perfect for use as prompts in text-to-image tools. Try copying a concept and using it in our Sketch to Image or Vector Style Image generators."
      },
      {
        q: "What kind of themes work best?",
        a: "Be descriptive and evocative! Try combining concepts, like 'A cozy, rainy day in a London library' or 'A vibrant, retro 80s arcade'. The more creative your theme, the more interesting the ideas will be."
      },
  ];
  ...
  return (
    ...
    <div className="animate-fade-in -m-6 md:-m-8">
        ...
        <section className="text-center py-12 px-4 bg-background">
          <div className="max-w-4xl mx-auto">
            <h1 className="text-4xl sm:text-5xl font-extrabold text-text mb-4 leading-tight">
              Your Personal AI <span className="text-primary">Art Director</span>.
            </h1>
            <p className="max-w-2xl mx-auto text-lg text-muted mb-8">
             Stuck for illustration ideas? Enter any theme and get three unique, detailed concepts with style, color, and composition suggestions to inspire your next creation.
            </p>
             <button onClick={() => document.getElementById('illustration-tool')?.scrollIntoView({ behavior: 'smooth' })} className="bg-primary text-white font-bold py-3 px-8 rounded-lg transition-transform hover:scale-105 shadow-lg">
                Get Inspired
            </button>
          </div>
        </section>
        ...
        {/* The Tool Section */}
        <section id="illustration-tool" className="py-16 bg-surface px-4">
          <div className="container mx-auto max-w-4xl flex flex-col items-center">
            <div className="w-full flex justify-between items-center mb-4">
              <h2 className="text-3xl font-bold">Illustration Idea Generator</h2>
            ...
            <div className="w-full max-w-xl flex flex-col gap-2 mb-6">
                <label htmlFor="theme-input" className="text-left font-semibold">Enter a theme or concept:</label>
                <input
                    id="theme-input"
                    type="text"
                    value={theme}
                    ...
                    placeholder="e.g., 'Cyberpunk City at Night', 'Enchanted Forest'"
        ...
        <section className="py-16 bg-surface px-4">
          <div className="container mx-auto text-center">
             <h2 className="text-3xl font-bold mb-10">From a Spark to a Masterpiece</h2>
        ...
        <section className="py-16 text-center bg-surface px-4">
            <div className="container mx-auto">
                ...
                <p className="text-muted max-w-xl mx-auto mb-8">Stop waiting for inspiration to strike. Generate it on demand and bring your vision to life.</p>
                <button onClick={() => document.getElementById('illustration-tool')?.scrollIntoView({ behavior: 'smooth' })} className="bg-primary text-white font-bold py-3 px-8 rounded-lg transition-transform hover:scale-105 shadow-lg">
    ...
    <div className={`fixed top-0 right-0 h-full w-full max-w-sm bg-surface shadow-2xl transform transition-transform duration-300 ${showHistory ? 'translate-x-0' : 'translate-x-full'} z-50 flex flex-col`}>
        ...
        <div className="flex-grow overflow-y-auto p-4">
            {savedItems.length > 0 ? (
                        ...
                        <li key={item.id} className="bg-background border border-border p-3 rounded-lg group">
                            <div className="flex items-start gap-3">
                              <div className="flex-grow overflow-hidden">
                                <p className="font-semibold text-text truncate">{item.theme}</p>
                ...
                <p className="text-center text-muted mt-8">No saved illustration ideas yet.</p>
            )}
        </div>
    </div>
