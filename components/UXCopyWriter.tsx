...
import { generateUXCopy } from '../services/geminiService';
import { UXCopy } from '../types';
...
interface UXCopyWriterProps {
  isAuthenticated: boolean;
  onAuthRequired: () => void;
}

export const UXCopyWriter: React.FC<UXCopyWriterProps> = ({ isAuthenticated, onAuthRequired }) => {
  const [componentDescription, setComponentDescription] = useState<string>('');
  const [copyOptions, setCopyOptions] = useState<UXCopy[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [copySuccess, setCopySuccess] = useState(false);

  const [savedItems, setSavedItems] = useState<SavedUXCopy[]>([]);
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem('savedUXCopy');
  ...
  useEffect(() => {
    if (!isInitialLoad) {
      localStorage.setItem('savedUXCopy', JSON.stringify(savedItems));
    }
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateUXCopy(componentDescription);
  ...
  const handleLoad = (item: SavedUXCopy) => {
    setComponentDescription(item.componentDescription);
    setCopyOptions(item.copyOptions);
    setCurrentItemId(item.id);
    setShowHistory(false);
    document.getElementById('ux-tool')?.scrollIntoView({ behavior: 'smooth' });
  };
  ...
  const handleCopy = () => {
    const textToCopy = copyOptions.map(c => `Option: ${c.option}\nRationale: ${c.rationale}`).join('\n\n');
  ...
  const handleDownload = () => {
    const textToDownload = copyOptions.map(c => `Option: ${c.option}\nRationale: ${c.rationale}`).join('\n\n');
    const blob = new Blob([textToDownload], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `ux_copy_${Date.now()}.txt`;
    a.click();
  ...
  const features = [
      {
        ...
        description: "Get three distinct microcopy options for any UI component, complete with a rationale for why each is effective."
      },
      {
        ...
        description: "Good microcopy guides users, prevents errors, and builds trust. Elevate your design with words that are clear, concise, and helpful."
      },
      {
        ...
        description: "Stop agonizing over the perfect wording. Get expert-level suggestions in seconds so you can focus on the bigger picture."
      },
  ];

  const testimonials = [
      {
        quote: "This is an absolute lifesaver. I used to spend way too much time trying to write the perfect button text. Now I get great options instantly. Can't imagine my workflow without it.",
        name: "Chloe Kim",
        title: "Product Designer"
      },
      {
        quote: "As a developer, writing good UX copy is not my strong suit. This tool helps me fill in the gaps with professional, user-friendly text that makes my apps better.",
        name: "James Riley",
   ...
   const faqs = [
      {
        q: "What is 'microcopy'?",
        a: "Microcopy refers to the small pieces of text in a user interface that help guide the user, such as button labels, error messages, tooltips, and placeholder text. Good microcopy can significantly improve usability."
      },
      {
        q: "What kind of component description should I provide?",
        a: "Be specific about the component and the user's goal. For example, instead of 'delete button', try 'A confirmation button to permanently delete a user's project from their account'."
      },
      {
        q: "Can I use the generated copy directly in my designs?",
        a: "Yes! The copy is generated to be used directly, but we always recommend reviewing it to ensure it perfectly fits your brand's tone of voice and specific context."
      },
  ];
  ...
  return (
    ...
    <div className="animate-fade-in -m-6 md:-m-8">
        ...
        <section className="text-center py-12 px-4 bg-background">
          <div className="max-w-4xl mx-auto">
            <h1 className="text-4xl sm:text-5xl font-extrabold text-text mb-4 leading-tight">
              Write Words That Work. <span className="text-primary">Effortlessly.</span>
            </h1>
            <p className="max-w-2xl mx-auto text-lg text-muted mb-8">
             Great design deserves great copy. Get sharp, clear, and effective microcopy for your buttons, tooltips, and UI elements in seconds.
            </p>
             <button onClick={() => document.getElementById('ux-tool')?.scrollIntoView({ behavior: 'smooth' })} className="bg-primary text-white font-bold py-3 px-8 rounded-lg transition-transform hover:scale-105 shadow-lg">
                Write My First Copy
            </button>
          </div>
        </section>
        ...
        {/* The Tool Section */}
        <section id="ux-tool" className="py-16 bg-surface px-4">
          <div className="container mx-auto max-w-4xl flex flex-col items-center">
            <div className="w-full flex justify-between items-center mb-4">
              <h2 className="text-3xl font-bold">UX Copy Writer</h2>
            ...
            <div className="w-full max-w-2xl flex flex-col gap-4 mb-6">
              <textarea
                ...
                placeholder="e.g., 'A confirmation button to permanently delete a user's project.'"
                rows={3}
            ...
            {copyOptions.length > 0 && (
              <div className="w-full max-w-2xl">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-semibold text-text">Generated Options</h3>
                  ...
                  <div className="flex flex-col gap-4">
                    {copyOptions.map((copy, index) => (
                      <Card key={index}>
                        <p className="text-xl font-bold text-text bg-background inline-block px-3 py-1 rounded-md mb-2 border border-border">
                          {copy.option}
                        </p>
                        <p className="text-muted">{copy.rationale}</p>
                      </Card>
                    ))}
                  </div>
              </div>
        ...
        <section className="py-16 bg-surface px-4">
          <div className="container mx-auto text-center">
             <h2 className="text-3xl font-bold mb-10">The Designer's Choice for Copy</h2>
        ...
        <section className="py-16 text-center bg-surface px-4">
            <div className="container mx-auto">
                <h2 className="text-3xl font-bold mb-4">Ready to Perfect Your Product's Voice?</h2>
                <p className="text-muted max-w-xl mx-auto mb-8">Stop guessing and start guiding your users with clear, effective copy. Generate your first option now.</p>
                <button onClick={() => document.getElementById('ux-tool')?.scrollIntoView({ behavior: 'smooth' })} className="bg-primary text-white font-bold py-3 px-8 rounded-lg transition-transform hover:scale-105 shadow-lg">
                    Generate UX Copy
                </button>
            </div>
    ...
    <div className={`fixed top-0 right-0 h-full w-full max-w-sm bg-surface shadow-2xl transform transition-transform duration-300 ${showHistory ? 'translate-x-0' : 'translate-x-full'} z-50 flex flex-col`}>
        ...
        <div className="flex-grow overflow-y-auto p-4">
            {savedItems.length > 0 ? (
                        ...
                        <li key={item.id} className="bg-background border border-border p-3 rounded-lg group">
                            <div className="flex items-start gap-3">
                              <div className="flex-grow overflow-hidden">
                                <p className="font-semibold text-text truncate">{item.componentDescription}</p>
                ...
                <p className="text-center text-muted mt-8">No saved UX copy yet.</p>
            )}
        </div>
    </div>
